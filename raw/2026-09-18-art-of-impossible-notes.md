...
capture_note: >
  **本字段只记「抓取事实」，不记「阅读结论」。**（AGENTS.md 1.1）
  ...
  素材的分段、行区间、AI 生成段判定、伪引注清单一律维护在
  wiki/sources/2026-09-18-art-of-impossible-notes.md —— 它们会随阅读深入而变，
  而本文件不可变，写在这里必然产生死结。

  **更正记录（2026-09-18，人类裁定，按 AGENTS.md 1.1 例外条款执行）：**
  本字段原含一张三段分层表与一份缺陷清单，二者均已作废。**旧值保留如下，不删除**
  （1.1 第 4 条：可更正 ≠ 可覆盖）：
  - 原记「A 1–443 ｜ B 445–1955 ｜ C 2011–2196」—— **作废**。作废有两个理由：
    **①** 它用的是「跳过 frontmatter 的内容相对行号」，而 wiki 页按「文件绝对行号」引用，
    **于是每一条下游引用都偏了 39 行**；**②** 分段本身也已从三段改为六段
    —— 原表把成书正文与 AI 生成的后记并在了一段里。
  - 原记「A 段含 4 处伪引注」—— **作废**（实为 5 处，且位置不在 A 段）。
  - 原记的「A 段判为 AI 生成的四条依据」与「已知缺陷」清单 —— **移至 sources 页**。**注意：本次更正本身使本文件从 2235 行增至 2238 行（+3），全部行区间随之平移 +3** —— 这是 1.1 未预见的情况：**更正元数据会改变行号，从而使所有下游行区间失效**。已重新测量并同步全部下游引用。**教训：更正 raw 时应尽量用等行数替换。**

  **本文件不再维护行区间与分层结论。** 正确值见 sources 页（已与实际文件逐行核对，
  文件共 2236 行）。**任何行区间断言前请回文件核对，不要引用本字段的旧值。**

  **已知缺陷：见 sources 页。** 原列于此的缺陷清单（伪引注、「高能量姿势」的重复性争议、
  无出处数字）属「阅读结论」，已按 1.1 的字段归位原则移出本文件。
  后续页面不得把本文件的任何段落当作证据引用 —— 请引用 sources 页的分层结论。

  **利益披露：无。** 文档内唯一外链是得到电子书的阅读入口，无 UTM、无返利参数，未发现推广性质。整理者未署名 —— 这意味着**可追溯性为零**（区别于「能追溯但动机有偏」的推广内容）。

  **版权性质**：正文主干为成书正文的完整搬运，非整理者原创；其余段落为二手加工。本文件以个人研究用途存档于本地 `raw/`，不对外发布。
---
