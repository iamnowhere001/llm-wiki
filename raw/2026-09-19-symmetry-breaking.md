...
capture_method: >
  ...
  document_id: A7vBdWbANouQ65x2OhRcv3fwnYd ｜ revision_id: 91 ｜ 正文 6,808 字符 / 190 行（均指本文件去掉 frontmatter 之后的正文）。
  结构化端点抓取：标题层级、行内链接、加粗标记、引用块均保留；未做 HTML 渲染，无 `→` 转 `->` 一类损失。
  内嵌图片 0/5 张已下载，→ raw/assets/2026-09-19-symmetry-breaking-img-NN.*，序号按正文出现顺序。
...
![图片展示的是太空望远镜拍摄的星空照片，画面中无数颗星星密密麻麻铺满画面，它们的位置看上去完全随机。但每个小光点其实是一个星系，照片上一毫米的距离就是几亿光年。这张图片与上下文紧密相关，用以说明太空望远镜拍摄的星空照片中看似随机的星星位置，实际上每个光点都是一个星系，强调了其复杂性，呼应了上文提到的复杂性科学思想工具这一主题。](https://feishu.cn/file/H8YebBKOnoPVWPxu0Vac3fG2nEc)
...
![图片为科幻漫画风格，标题为“第十亿零一个”。画面中，一位身穿白大褂的女性与一位穿着制服的男性在宇宙模拟室里对话，男性指着屏幕上的数字，女性则看向他。接着画面切换到宇宙中，出现“十亿对十亿——归零”字样。随后女性手掌中出现一个发光的球体，男性看向她。最后画面展示宇宙中星系、行星等景象，底部有“对称性破缺”字样。此图与上下文探讨宇宙中普通物质来源及对称性破缺相关，以漫画形式呈现相关概念。](https://feishu.cn/file/FKcQbsi6noiFUIx1SQLcVAIPncg)
...
![图片展示了序参量概念，标题为“序参量：给破缺装一个仪表”。横轴为时间，纵轴为全选A和全选B。图中曲线代表系统走向，标注有均衡、出手窗口、正反馈区、锁定等关键区域。曲线在均衡区域波动，出手窗口时走向已可读锁定未完成，正反馈区越偏越快，锁定后路径依赖回头越来越贵。下方文字说明先看走向再看大小，差值变化率比存量更早报信，读数=0且摆幅变慢偏得变久就应盯紧。](https://feishu.cn/file/LqcCbLSRGogvtLxN8Eac4Edensd)
...
![图片展示了对称性破缺时系统状态随温度变化的曲线图。横轴为温度，从左至右依次为太热、机会窗口、已冷。纵轴有三个指标：①可塑性（响应度）；②序参量；③回响时长（临界慢化）。在机会窗口处，可塑性、序参量和回响时长均达到峰值。该图与上下文紧密相关，通过可视化呈现系统在不同温度下的状态变化，帮助理解临近对称性破缺时系统行为的转变。](https://feishu.cn/file/COiFbRBBLoD4uQxYBqycqZvbnwc)
...
![图片为“对称性破缺：命运不过是冷却了的偶然”相关内容的海报。海报上方介绍对称性破缺概念，称其是宇宙从热到冷的冷却过程。中间部分以宇宙、人类社会、技术发展等为例，阐述对称性破缺现象。下方列举了利用对称性破缺的三大心态、三条路径、三条方法，以及生活中的应用场景。底部有偈赞“万古千秋一梦中，休言天地或人同，就有飞鸿踏火，犹存喘息烟红”，并标注“方维夏 / 近代数学奠基”。](https://feishu.cn/file/NOzVbqz7korCozx2VyEc5b2inWf)