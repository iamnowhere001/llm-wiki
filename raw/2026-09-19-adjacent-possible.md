...
author: 万维钢（得到课程讲稿）；收尾「又曰」代笔者未披露（得到课程惯例），判定见对应 sources 页
...
kind: course-article
series: 得到《现代思维工具课》「模块七 演化者」正文｜讲次表编号（推定）099
...
capture_method: >
  ...
  document_id: G7yKdKZ3IoBq57xHMtwcfBiZn9g ｜ revision_id: 112 ｜ 正文 7,079 字符 / 201 行（均指本文件去掉 frontmatter 之后的正文）。
  结构化端点抓取：标题层级、行内链接、加粗标记、引用块均保留；未做 HTML 渲染，无 `→` 转 `->` 一类损失。
  内嵌图片 0/6 张已下载，→ raw/assets/2026-09-19-adjacent-possible-img-NN.*，序号按正文出现顺序。
...
![图片以“临近可能：五十五岁的下一扇门”为标题，展示了张忠谋在台积电不同阶段的场景。52岁时离开德州仪器，55岁时押上一座工厂，面对未知的生意，60岁以后迎来黄金时代。画面中张忠谋身着西装，或在办公室思考，或在会议室讲解，或站在工厂前，还有他与台积电员工的场景，以及台积电的芯片图示。图片与上下文紧密相关，直观呈现了张忠谋在台积电的奋斗历程。](https://feishu.cn/file/BO1pbVqFXozZ96xRKLscHTR8nWf)
...
![图片展示了临近可能的概念，分为“此刻·t”和“行动之后·t + 1”两个阶段。左侧“此刻·t”中，深色区域代表已实现状态，周围橙色区域为邻近可能，灰色区域为尚不可及。右侧“行动之后·t + 1”中，邻近可能区域扩大，新增红色圆圈标注的“新的门”，表明通过行动可实现新状态。图片与上下文紧密相关，直观呈现了临近可能随行动而变化的特性。](https://feishu.cn/file/LP3ebRllGovn73xL9a8c6oWmnTg)
...
![图片展示了“临近可能”的结构，以魔法房子为喻。实线房间代表已实现，实线门代表此刻够得着，虚线门代表走进去之后才会生成。图中还标注了“这一层的门：数得清，谁站得近谁先推”，以及“下一层的门：这间房没人走进去之前，并”。图片与上下文紧密相关，直观呈现了临近可能学说中创新可能性的实现与生成，强调实现一种可能性才能打开新的可能性。](https://feishu.cn/file/ChvSbthPSoukAsxPU8Lc7PqxnJe)
...
![图片为漫画《现实构件》，展示了甲和乙在现实世界中制作和使用工具的过程。甲拿着一张纸，乙在制作红色机械装置，甲说“先把这个烂东西做出来”。乙完成装置后，甲说“这个功能，我真需要”，乙说“一起做下一面？”，两人合作制作出彩色建筑模型。漫画通过这一系列场景，与上下文关于实现无法事先想象事情的讨论相呼应，强调了现实中的尝试与合作。](https://feishu.cn/file/QUP8bMMBhorQpaxUqwrc3Lf3npb)
...
![图片展示了“你 = 可能性空间中的一个位置”的概念。画面中有“现在的你（A）”“临近的一步”“走一步后的你（B）”三个位置，以及从A点够得着的门和红色的门。文字说明位置的坐标包括能力、关系、经历、所在之处，每一项都可以挪动，性格测试测出的是A点的读数，不是你这个人的常量。该图与上下文紧密相关，直观呈现了人处于不同位置时所对应的可能性空间变化。](https://feishu.cn/file/WGB3bmi8ZoriXexiWe9cs3ALnud)
...
![图片是一张名为“临近可能：如何实现无法事先想象的事情？”的海报，介绍了临近可能的概念及实现方法。海报上方有“临近可能”字样，下方列出临近可能的定义、工具、原因、步骤等内容。中间部分展示了“临近可能”与“现有状态”的对比，以及“临近可能”实现的三个步骤。底部有“临近可能”应用的场景，如科学、艺术、商业等。该图与上下文紧密相关，是对临近可能概念及实现方法的详细阐述。](https://feishu.cn/file/BdiPbECoHoJpI4xOljecQSNincf)