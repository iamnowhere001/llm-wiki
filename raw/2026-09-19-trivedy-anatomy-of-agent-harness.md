...
disclosure: >
  ⚠️ **利益披露**：作者为 LangChain 员工，文末含 LangSmith（LangChain 自家 Agent 工程平台）
  的推广段「See what your agent is really doing / Try LangSmith」。按 schema §3.4，
  引用本素材的页面须降 confidence。
capture_method: >
  curl 直取静态 HTML（173,209 字节），用自写零依赖提取器按 <main> 边界还原结构。
  抓取时点 2026-09-19。
capture_note: >
  ⚠️ **本文件是 `Agent = Model + Harness` 这个定义的原始出处**，且该定义出现在一节标题为
  「Can Someone Please Define a "Harness"?」之下 —— **说明它是一次「补定义」的动作，
  提出时业界并无共识**。原句：If you're not the model, you're the harness.
  这一点对本库「定义」维度很关键：**定义的权威性来自使用者的引用，不来自任何标准组织。**
  **已知缺失**：① 作者头像与社交链接在提取时被删除；② 文末推广段保留（已在上方 disclosure 标注）。
---

# The Anatomy of an Agent Harness

Vivek Trivedy · March 10, 202612min[Go back to blog](/blog)Create agentsShare
...
Open SourceAgent ArchitectureLangChain

#### Building a Harness with Jev

Sydney RunkleHunter LovellSeptember 17, 20265min[](/blog/building-a-harness-with-jev)Tutorials & How-TosAgent Architecture

#### How we built LangChain's Paid Media Agent

Amal IrgashevDanny LambertJan GomezSeptember 13, 202619min[](/blog/paid-media-agent)Agent ArchitectureDeep AgentsOpen Source

#### Organizing Context in a Multi-Agent Harness

Thushanth BengreChester CurmeSeptember 8, 20266min[](/blog/organizing-context-in-a-multi-agent-harness)Thank you! Your submission has been received!Oops! Something went wrong while submitting the form.
