...
capture_method: >
  curl 直取静态 HTML（40,733 字节，无 JS 渲染层），用自写零依赖提取器
  按 <main> 边界还原结构。抓取时点 2026-09-19。
capture_note: >
  ⚠️ **署名纠错（本库第一处）**：本页挂在 martinfowler.com 域名下，但**作者是 Birgitta Böckeler**
  （Thoughtworks 的 Distinguished Engineer），不是 Martin Fowler。本库此前引用的中文转述
  多把此文记作「Fowler 的文章」—— 正确说法是「**Fowler 站点上、由 Böckeler 署名的文章**」。
  **已知缺失**：① 页面顶部的作者照片与简介块在提取时被删除（作者信息已入 frontmatter）；
  ② 文末的修订历史（含 2026-02-17 的初版 memo）保留在正文末尾。
  **内容要点**：本文是「guides / sensors」二分与「computational vs inferential」维度的出处，
  并提出 harness 的三类 regulation category（maintainability / architecture fitness / behaviour）。
---
...
- Regulation categories - Maintainability harness - Architecture fitness harness - Behaviour harness
- Harnessability
- Harness templates
...
Figure 1:
 The term “harness” means different things depending on the bounded context.
...
My colleague Ned Letcher uses the term “ambient affordances” for
...
I wrote [a follow-up article](/articles/sensors-for-coding-agents.html) describing my experiments and learnings with sensors for maintainability.
