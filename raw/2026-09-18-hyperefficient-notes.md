...
capture_note: >
  **本字段只记「抓取事实」，不记「阅读结论」。**（AGENTS.md 1.1）
  ...
  素材的分段、行区间、AI 整理段判定、弱引注清单一律维护在
  wiki/sources/2026-09-18-hyperefficient-notes.md —— 它们会随阅读深入而变，
  而本文件不可变，写在这里必然产生死结。

  **更正记录（2026-09-18，人类裁定，按 AGENTS.md 1.1 例外条款执行）：**
  本字段原含一张四段分层表、一份「A 段判据」、一份逐行核对结论与一份缺陷清单，
  均为「阅读结论」，已移出本文件。**旧值保留如下，不删除**（1.1 第 4 条）：
  - 原记「A 1–115 ｜ B 116–128 ｜ C 130–431 ｜ D 435–441」—— **作废**。
    病因与 2026-09-18-art-of-impossible-notes 完全相同：用的是**跳过 frontmatter
    的内容相对行号**（`capture_method` 里的「441 行」即该坐标系的总行数），
    而 wiki 页按**文件绝对行号**引用，**每条下游引用都偏了一个 frontmatter 的长度**。
  - 原记的「A 段判为整理的四条依据」、「逐行核对结论」（摘要段做减法不做加法）
    与「已知缺陷」清单 —— **全部移至 sources 页**。

  **本文件不再维护行区间与分层结论。** 正确值见 sources 页（已与实际文件逐行核对）。
  **任何行区间断言前请回文件核对，不要引用本字段的旧值。**

  **利益披露：** 文档内唯一外链是得到课程的三篇文章链接（`dedao.cn/course/article?id=...`），**无 UTM、无返利参数**，未发现推广性质。
  但需注意：**讲稿段是得到付费课程讲稿的完整搬运**，参考资料直接指向该课程的售卖入口 —— 这是内容来源的性质，不是整理者的推广行为。两件事应分开标注。

  **版权性质：** 讲稿段（约 300 行）是付费课程讲稿的完整搬运，编号要点段是讲稿摘录，摘要段是对讲稿的二次加工。三者均非整理者原创。本文件以个人研究用途存档于本地 `raw/`，不对外发布。
