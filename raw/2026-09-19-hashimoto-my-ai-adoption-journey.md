...
capture_method: >
  curl 直取静态 HTML（65,498 字节，无 JS 渲染层），用自写零依赖提取器
  按 <main> 边界还原结构：标题层级、行内链接、加粗、列表、脚注全部保留。
  抓取时点 2026-09-19。
capture_note: >
  ⚠️ **本文件是「Harness Engineering」这个词的命名现场。** 第 5 步「Engineer the Harness」
  是 Mitchell Hashimoto 首次使用该词之处，原文自陈「I don't know if there is a broad
  industry-accepted term for this yet」（我不知道业界是否已有公认术语）。
  **已知缺失**：① 正文的视觉硬换行未完全重组（原文按约 70 字符换行，提取后部分句子仍跨行）；
  ② 页面导航与「Table of Contents」列表在提取时被保留，未与正文区分。
  **作者自述**：文末脚注声明「I don't work for, invest in, or advise any AI companies」（无利益关联）。
---
...
[Claude Code](https://github.com/anthropics/claude-code). I'll cut to the
...
the highly publicized [Anthropic skill formation paper](https://www.anthropic.com/research/AI-assistance-coding-skills).
...
models like Amp's [deep mode](https://ampcode.com/news/deep-mode) (which
