...
capture_method: >
  ...
  document_id: EaCbdTaKCobftTxNNnzcmElwnfb ｜ revision_id: 76 ｜ 正文 6,844 字符 / 188 行（均指本文件去掉 frontmatter 之后的正文）。
  结构化端点抓取：标题层级、行内链接、加粗标记、引用块均保留；未做 HTML 渲染，无 `→` 转 `->` 一类损失。
  内嵌图片 0/6 张已下载，→ raw/assets/2026-09-19-self-organized-criticality-img-NN.*，序号按正文出现顺序。
...
![图片展示了系统在亚临界、临界和超临界三种状态下的事件传播情况。亚临界状态（R < 1）中，扰动很快熄灭，平均每次只能带动少于1个后续事件；临界状态（R ≈ 1）下，扰动就在门槛上，平均每次大约带动1个后续事件，小事可能小，小事也可能变大；超临界状态（R > 1）中，扰动会失控蔓延，平均每次带动多于1个后续事件。图片与上下文紧密相关，直观呈现了系统在不同状态下的事件传播特性。](https://feishu.cn/file/BbHbbUffAoiXavx54CncoQrknXd)
...
![图片展示了沙堆模型实验过程。上方有“慢慢加沙”“逼近临界”“快速雪崩”“边缘耗散”“再次积累”等步骤标识。实验中，沙子从上方漏斗不断加入，形成沙堆，当沙堆接近临界状态时，会引发快速雪崩，沙子从边缘耗散，再重新积累，如此循环。该图与上下文介绍的自组织临界机制相关，直观呈现了系统内部自带平衡回路，自动调节状态的原理。](https://feishu.cn/file/WqXdbe2o9otJelxQrW8cnH9NnIg)
...
![图片展示了自组织临界系统如何自己回到临界状态的循环过程。从慢变量悄悄积累开始，积累到一定程度触发雪崩，快变量猛然释放，释放后系统落回临界附近再开始新一轮积累。图中用不同颜色和箭头标识了四个关键步骤，还标注了临界是系统自动回归的地方，以及不靠人扶，靠负反馈自己转起来的原理。该图与上下文关于自组织临界的心法相呼应，直观呈现了系统自我调节的机制。](https://feishu.cn/file/RxeZblknBoVyvmxIjjJc3BE9n0g)
...
![图片展示了两种在积累压力时的活法。上方为“临界”状态，常有小崩溃，每次泄掉一点，压力始终顶到上限之下就回落；下方为“高压亚临界”状态，小崩溃全被压住，负荷在底下偷偷涨过头，崩就崩一个大的。图片与上下文紧密相关，上下文提到研究发现最幸福状态偏向“忙”的一头，即手头工作一直有点满，时时顶着点劲，可又没被压垮的状态，此图形象地呈现了这种状态下的压力积累与释放情况。](https://feishu.cn/file/QOFDbmsE6oVbHox1JoMcfLDVn1c)



2021 年的一项研究分析了三万五千多人的数据，还做了两个实验，专门考察一个人“可自由支配的空闲时间”和幸福感的关系。结论是一条倒 U 型的曲线：**空闲太少，被事情追着跑，不幸福；但空闲太多，无所事事、日子空落落的，也不幸福。**

![图片是一张关于“可自由支配的空闲时间”与幸福感关系的图表。横轴为可自由支配的空闲时间（单位：小时），纵轴为主观幸福感。柱状图代表不同空闲时间下的幸福感平均值，蓝色曲线为编辑示意图。研究发现，幸福感呈现倒U型曲线，最佳区间约2 - 5小时，偏向忙碌的一侧，即手头工作一直有点满，时时顶着点劲，但又没被压垮的状态。](https://feishu.cn/file/IhABbFjzVoDd2vxBUIMcmQScnph)
...
![图片为“自组织临界：恰到好处的活泼”相关内容图示。上方介绍自组织临界（SOC）概念，核心是“一个动态系统在多个参数达到临界点时，会突然发生质变”。中间部分以沙堆模型为例，阐述临界系统的三大特性，包括信息传递、复杂生长出结构、自组织临界。下方有“有偈为证”板块，引用“死水无波难结冰，偏于沉时易泛风。小火频频煮不老，余烬一瞬便成灰”等诗句。整体是对自组织临界概念及特性的图文说明。](https://feishu.cn/file/P1h2bvY6AoIZKFxEG74cnrhvnvc)