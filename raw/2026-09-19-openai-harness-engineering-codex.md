...
author: Ryan Lopopolo（OpenAI, Member of the Technical Staff）
...
capture_method: >
  ⚠️ **本文件经 AI 中介提取，非直接抓取。** 官方站对 curl 一律返回 403（Cloudflare 拦截）——
  已试过 4 种 UA / header 组合、HTTP/1.1、去掉 www 的变体，全部 403；
  存档站（web.archive.org、archive.org 的 availability API）与公开代理（r.jina.ai）
  在本环境被沙箱阻断（CONNECT tunnel failed / connection timed out）。故改用 WebFetch 提取。
capture_note: >
  **交叉验证（已做，但未达逐字比对）**：
  ① 作者 —— 独立检索到第三方来源（wiki.lkwplus.com）称其为「A primary-source post from
  **Ryan Lopopolo** at openai」，与提取结果一致；
  ② **一处转载错误被纠正**：engineering.fyi 的转载页把作者写成「OpenAI's Harness team」——
  那是转载者自己的概括，原文署名为**个人**（Ryan Lopopolo, Member of the Technical Staff）。
  **未做到的**：没能拿到原始 HTML 做逐字比对，故 capture_quality 记 **medium** 而非 high。
  **⚠️ 本文的核心性质（本库判断，待真正的一手核对）**：文章标题用了 "Harness engineering"，
  但**正文中 "harness" 一词几乎不再出现** —— 除标题外仅有一处技术性用法
  「Evaluation harnesses」（出现在「Agents produce」清单里）。**正文从未给出 harness 的显式定义**
  （没有 "By harness we mean…" 或 "A harness is…" 这类句子）。
  这与中文圈「OpenAI 提出了 Harness Engineering」的普遍说法**有实质差距**：
  **文章命名了它，但没有定义它。**
  **已知缺失**：① 仓库知识库目录结构的代码块被中介省略（正文中已标注位置）；
  ② 小节锚点列表未保留；③ 提取器未做逐字校验。
---

# Harness engineering: leveraging Codex in an agent-first world

**Ryan Lopopolo, Member of the Technical Staff** ｜ February 11, 2026
...
> ⚠️ **提取缺失**：此处原文有一个「仓库知识库目录结构」的代码块，AI 中介未完整保留。
...
> ⚠️ **提取注**：上表中的「Evaluation harnesses」是**全文正文里唯一一次出现 "harness"**（标题除外）。
