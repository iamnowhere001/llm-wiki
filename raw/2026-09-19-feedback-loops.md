---
title: "反馈回路：怎样操控复杂系统"
author: 万维钢（得到课程讲稿）；本节点另有同名子文档（关联档案，已单独落档），判定见对应 sources 页
...
capture_method: >
  ...
  document_id: W9uudMqYSoVVEtxngJIcxRvlnSc ｜ revision_id: 85 ｜ 正文 6,526 字符 / 153 行（均指本文件去掉 frontmatter 之后的正文）。
  结构化端点抓取：标题层级、行内链接、加粗标记、引用块均保留；未做 HTML 渲染，无 `→` 转 `->` 一类损失。
  内嵌图片 0/4 张已下载，→ raw/assets/2026-09-19-feedback-loops-img-NN.*，序号按正文出现顺序。
...
![图片展示了强化回路与平衡回路的概念。强化回路R以红色箭头表示，包含“降价”“销量增加”“成本摊薄”三个环节，形成越转越大的循环。平衡回路B以蓝色箭头表示，包含“室温升高”“降温后停机”“空调制冷”等环节，目标是拉回室温。图片还通过曲线图说明，强化回路导致数字一路指数狂奔，平衡回路使曲线绕着一条线小幅波动。该图与上下文紧密相关，直观呈现了强化回路与平衡回路的运作机制。](https://feishu.cn/file/FDNsb5216ozQzxxjbtFcCyafnWg)
...
![图片展示了快回路挤压慢回路的机制。进度落后时，全员加班（快回路，以天计）使进度暂时追上，但被大把抽走；同时，改流程、练兵、自动化（慢回路，以月计）使火源变少，但所剩无几。两条回路治同一个病，却在抢同一份资源，最终火更多，需靠加班，慢回路断粮。图片与上下文紧密相关，直观呈现了压力越大，快回路挤压慢回路，公司越不聪明，越依赖救火的恶性循环。](https://feishu.cn/file/ZcEGbW5F8oncyjx3DNMcoC3Bnih)
...
![图片是一张关于反馈回路的图表，标题为“反馈回路：怎样操控复杂系统”。图表分为强化回路（正反馈）和平衡回路（负反馈）两大类，分别阐述了其作用及构成。强化回路通过A、B、C三个环节放大变化，如“大土”“大水”等；平衡回路则通过A、B、C三个环节纠正偏差，如“大风”“大浪”等。图表还列举了生活中的例子，如“生活中的房子”“股票”等，并配有相关公式和文字说明。该图与上下文紧密相关，是对反馈回路概念的直观呈现。](https://feishu.cn/file/QGvSbyNaQoGlcWx1Wf5cnUianHf)