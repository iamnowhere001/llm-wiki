...
## 第二份素材：同一个比喻的通俗版，强度更低

[[2026-09-18-modern-thinking-tools-prospectus]] 的 U 段（**AI 生成段**）用本页这个比喻解释另一件事：
...
- **与 [[granularity-causal-mediation]]：MDL 是本页的形式化。** 本页说「**理解即压缩**」；
  该页行 97 把它落到**决策语境**，并给出形式化 —— **最小描述长度（MDL）= 模型本身的长度 + 数据补丁的长度**（该页）。
  **本页给命题，该页给可计算的表达。**

- **[[2026-09-19-zeroth-order-truth]]（107）**：本页判据在**决策语境**的实例 ——
  「**大局观是对信息做『有损压缩』。它忽略大部分内容，只要求保住三样东西：方向、量级和机制**」；
  配套句「大局和细节的区别，不是宏大与微小，而是**控制力与装饰性**」。
  与本页的区别：本页讲「智能 = 压缩」，107 讲**压缩是决策的必要动作**（不压缩就无法行动）。
- **[[2026-09-19-pretraining-posttraining]]（117）**：补一条**独见** ——「**工具不是经验的替代品，工具是经验的压缩算法**」。
  这是本页「压缩」概念在**工具观**上的落点：工具不增加信息，它压缩经验以便调用（该讲「思维工具是调用你庞大心智基座模型的快捷键」）。
