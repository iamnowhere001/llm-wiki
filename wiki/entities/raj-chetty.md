---
title: 拉杰·切蒂（Raj Chetty）
type: entity
slug: raj-chetty
tags: [人物, 经济学家, 代际流动, 社会资本, 哈佛]
created: 2026-09-18
updated: 2026-09-18
sources: [2026-09-18-social-capital-structural-holes-notes, 2026-09-18-five-thinking-tools-livestream]
related: [social-capital, structural-holes, opportunity-systems, multiplicative-world, luck-over-talent, modern-thinking-tools]
confidence: medium
status: active
---

# 拉杰·切蒂（Raj Chetty）

> 哈佛大学经济学家，**用超大规模行政数据（纳税记录、Facebook 关系图谱）研究代际流动与社会资本**。
> 他是本库目前**唯一一位「有可核论文」的经济学家** —— 014 讲引用的两组数字逐字核到原文。

- **类型**：人物（经济学家）
- **别名**：Raj Chetty
- **外部链接**：—

## 是什么

「特别擅长从超大规模的数据中挖掘社会的规律」（`raw/2026-09-18-social-capital-structural-holes-notes.md` 行 36）。
014 讲用他的两组研究支撑「位置论」，是本讲**证据最硬**的部分。

## 关键事实

| 时间 | 事实 | 来源 |
|---|---|---|
| 2014 | Chetty, Hendren, Kline & Saez, *Where is the Land of Opportunity?*, *QJE* 129(4):1553-1623 / NBER WP 19843 —— 贫困家庭孩子到 30 岁进入收入前 20% 的概率：**圣何塞 12.9% / 夏洛特 4.4%**（逐字属实） | 014 行 45 |
| 2018 | Chetty & Hendren, *The Effects of Neighborhoods on Intergenerational Mobility I*, *QJE* 133(3):1107-1162 —— 每在「好社区」多待一年，成年后「综合境遇」变好 **4%**（**方向已核，系数未取到原文 → 标「（未验证）」**） | 014 行 129 |
| 2022 | Chetty, Jackson, Kuchler, Stroebel et al., *Social Capital I*, *Nature* 608:108-121 —— 分析 **7220 万**名 25–44 岁用户、**210 亿**对好友关系、覆盖 **84%**；经济连通性高社区长大 → 成年收入平均 **+20%**（三个数字逐字属实） | 014 行 61 / 71 |

## 在本知识库中的角色

- **014 讲（[[social-capital]]）的全部硬证据来源**。收官直播再次回指：行 100「讲**结构洞**的时候提过」切蒂
  （[[2026-09-18-five-thinking-tools-livestream]] 回指核查表第 7 条）。
- **与 [[opportunity-systems]] 的关系**：那一页的发明家/收入数据同样出自切蒂团队，
  经 Grant《Hidden Potential》转载 —— **同一批研究的两种读法**：一个读成**制度问题**，一个读成**个人策略**。
- **与 [[luck-over-talent]] 的归因对照**：两者都在拆「个人奋斗论」，但一个归因到**位置**、一个归因到**运气**。
- **注意归属**：014 讲引用的 2025 年英国研究（**Harris**, Iyer, Rutter, Johnston et al.,
  *Social Capital in the United Kingdom*, 2000 万英国居民）**不是切蒂的论文**，不要误挂。

## 相关概念

- [[social-capital]] —— 本页是他的 2022 年研究在库内的落点
- [[structural-holes]] —— 014 讲横向连接的概念来自伯特，**不是切蒂**
- [[opportunity-systems]] —— 同一批数据的制度层读法

## 证据性质与待办

> [!note] 单来源声明 + 一处未核数字
> 本页的依据只有一份素材（014 讲）加一次直播回指，且**全部原始论文未收录进 `raw/`**。
> 其中 **「2018 年的 4%」系数未取到原文**，标「（未验证）」；
> 讲稿「他目前还没有得到诺贝尔经济学奖，但是呼声很高，将来肯定能得」（行 36）是**讲稿评价，未验证**。

- [ ] 取回 Chetty et al. 2018 *QJE* 133(3) 正文，确认 4% 的口径（收入百分位 vs 综合指数）
- [ ] 确认 2014 与 2022 两篇的识别策略（相关 vs 准因果）

## 来源

- [[2026-09-18-social-capital-structural-holes-notes]]（讲稿段行 35–147）
- [[2026-09-18-five-thinking-tools-livestream]]（行 100 的回指）
- 外部核查所用来源：Chetty et al. 2014 *QJE* 129(4)；Chetty & Hendren 2018 *QJE* 133(3)；
  Chetty, Jackson, Kuchler, Stroebel et al. 2022 *Nature* 608:108-121。**均未收录进 `raw/`**