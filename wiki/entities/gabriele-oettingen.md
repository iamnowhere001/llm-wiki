---
title: 加布里埃尔·厄廷根（Gabriele Oettingen）
type: entity
slug: gabriele-oettingen
tags: [人物, 研究者, 动机, WOOP, 心理比对]
created: 2026-09-18
updated: 2026-09-18
sources: [2026-09-18-woop-notes]
related: [woop, implementation-intentions, peter-gollwitzer, wishful-thinking-layers, self-determination-theory, 2026-09-18-woop-notes]
evidence_tier: single
confidence: medium
status: active
---

# 加布里埃尔·厄廷根（Gabriele Oettingen）

> **心理比对（Mental Contrasting）**的提出者，[[woop]] 四步流程的推广人。
> 013 讲一手引注的主体 —— **四组实证案例里有三组挂在她名下**。

- **类型**：人物（研究者）
- **机构**：纽约大学（NYU）心理学教授（[[2026-09-18-woop-notes]] 核查表 #6，**已核**）
- **外部链接**：https://woopmylife.org/ —— 其课题组的公开站点，也是 013 讲**唯一的对外链接**（素材注释 `[4]`，raw 行 207）

> [!warning] 孤证 —— 本页仅 1 份素材支撑
> 支撑本页的只有 [[2026-09-18-woop-notes]]，尚未获得第二份独立来源的交叉验证。
> **引用本页结论时应带着这个前提**，或先补一份独立来源把它升到 `crossed`。

## 是什么

心理学家。提出**心理比对**，并把它与执行意图打包成 [[woop]] 四步流程推广（raw 行 83）。她的书 *Rethinking Positive Thinking: Inside the New Science of Motivation*（2014, Penguin Random House）是这一方法的通俗化来源。

她的方法论位置很干净：**反「正能量心理学」**。幻想不是动力来源，反而**降低能量与血压**；必须在幻想之后立刻面对障碍（raw 行 103、105）。

## 关键事实

| 时间 | 事实 | 来源 |
|---|---|---|
| 2011 | Kappes & Oettingen：**6 分钟正向幻想即显著降低能量与血压** | 013 讲 raw 行 103 的主张；研究本身**真实存在**，但素材挂的 `[5]` 注释**缺失**（核查表 #7：**不可追溯**，非「不存在」） |
| 2013 | Duckworth, Kirby, Gollwitzer & Oettingen：**77 名城市中学五年级学生**，MCII 组成绩 η²=.07、出勤 η²=.05、行为 η²=.07 | 同页核查表 #9，**逐项命中** |
| 2013 | Sheeran, Harris, Vaughan, Oettingen & Gollwitzer, *Health Psychology* 32(7)：**英国北部钓鱼俱乐部 467 名中年、超重、低 SES 男性** | 同页核查表 #10 —— 013 讲把它写成「中年**女性**」，**性别对反**（使用失真，逐条公示） |
| 2014 | *Rethinking Positive Thinking*（Penguin Random House） | 同页核查表 #6；⚠️ 013 讲的图注（raw 行 111）把它写作《成功科学》，**中文译名待核** |
| 2021 | Monin, Oettingen, Laws et al.：45 位早期痴呆患者 + **90 位配偶照护者**；照护者感知压力 ↓（δ=1.71）、生活质量 ↑（δ=1.55）、正性情绪 ↑（δ=2.30） | 同页核查表 #12；⚠️ 013 讲添了「**抑郁指标**有所改善」，**原文显著项不含抑郁** |

## 在本知识库中的角色

- **是 [[woop]] 与 [[implementation-intentions]] 两页的引注锚点**：WOOP 的第一件武器（心理比对）完全是她这条线。
- **013 讲把她的结论讲对了、把其中两处细节讲歪了** —— 一处性别对反、一处多说了「抑郁」。这正是本库判 [[2026-09-18-woop-notes]] 为 `medium` 而非 `high` 的理由之一。
- **与 [[peter-gollwitzer]] 是夫妻** —— 013 讲 raw 行 113 明写「厄廷根的**丈夫**、著名心理学家彼得·戈尔维策」。多家来源一致，但**属二手印证**。
- **与 [[wishful-thinking-layers]] 有一处待核的同名关系**：013 素材把该页记的「幻想实现理论」称作心理比对的来源 —— **是同一理论的两个名字，还是两个理论？本库未裁定。**

## 相关概念

- [[woop]] —— 她推广的四步流程
- [[implementation-intentions]] —— 她丈夫提出的第二件武器（WOOP 的 P 步）
- [[peter-gollwitzer]] —— 配偶与合作者
- [[self-determination-theory]] —— ⚠️ 013 素材的 AI 段把 WOOP 接到 SDT 三需求上（该素材行 819），**不是她的主张**

## 来源

- [[2026-09-18-woop-notes]]（讲稿段行 83、103、111、129–145；注释 `[4]` 行 207。raw `2026-09-18-woop-notes.md`）
  **本页行号已回 raw 核对（文件绝对行号）。**
- 外部文献（**未收录进 `raw/`**）：Kappes & Oettingen (2011)；Duckworth et al. (2013)；Sheeran et al. (2013)；Monin et al. (2021)；*Rethinking Positive Thinking* (2014)。

> [!note] 单来源声明（`AGENTS.md` §7）
> 本页**全部内容**来自**一份素材**（得到课程讲稿的转述），本库**无一手续材料**（无原著、无论文原文）。其名下四项研究已在 sources 页逐条外部核到原文，**但那一步是 sources 页做的，不在 `raw/` 里**。