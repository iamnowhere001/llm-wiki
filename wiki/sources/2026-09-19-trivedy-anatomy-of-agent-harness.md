...
3. **TLDR **："Agent = Model + Harness. Harness engineering is how we build systems around models to turn them into work engines.
...
5. **两条 Key Takeaways（之后）**：规划工具让 agent 分解任务、跟踪进度并随学习调整；
   **派生 subagent 并行处理独立子任务，各自持有隔离的上下文**。
6. **上下文是贯穿全文的核心约束**：文中多次围绕 context window 展开。
