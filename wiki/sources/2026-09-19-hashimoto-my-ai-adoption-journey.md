...
4. **两种形式**：① **Better implicit prompting（AGENTS.md）**；② **Actual, programmed tools**（截图脚本、过滤测试等）。
5. **Ghostty 的 `AGENTS.md`**：作者称**每一行都基于一次 agent 的坏行为**，且「almost completely resolved them all」。
...
- **校准了中文二手材料的普遍说法**：中文材料普遍把 Harness Engineering 描述成一个 2026 年 2 月「突然火起来」的业界术语。
  原文显示：**命名者本人当时并不知道它会不会成为通用术语**。这**不构成对中文材料的证伪** ——
