...
页面总数 **65** ｜ 项目 **3**（进行中 3） ｜ 素材 **14** 份 ｜ 最后更新 2026-09-18
...
- [[viktor-frankl|维克多·弗兰克尔与「刺激与回应之间的间隙」]] — 本页 `confidence: low` 的原因不是弗兰克尔不重要，而是 —— **本库引用的那句话很可能不是他说的**。这是本项目中一个典型「广为流传但出处可疑」的引文案例。  `人物` `意义疗法` `情绪` `待核验`
...
- [[emotion-regulation-toolkit|情绪调节工具箱：素材给出的全部技术]] — 素材一里最有实用价值的部分，也是**证据最薄弱的部分** —— 一整套技术、句式和清单，几乎全部无引用来源。本页把它们集中归档，**并逐条标注来源层与待核状态**。  `情绪` `方法` `清单` `工具箱`
...
- [[internal-friction|内耗：定义、三种解释与它们的张力]] — 「内耗」是本项目最核心的待定义概念 —— 三份素材都用了它，**但给出了三种不同的定义**。这不是可以合并的差异，而是三种不同的病因学。  `情绪` `内耗` `反刍` `机制`
