...
**需要警惕的一点**：主线二的素材质量显著低于主线一。三份素材中，一份内部混层、一份是观点长文、**一份是纯 AI 生成且无任何来源**。本库因此第一次需要处理「AI 生成内容如何入库」这个问题 —— 见 [[mental-strength]] 的处理先例与 [[emotion-awareness]] 缺口表的第一条。
...
1. **AI 生成内容如何处理？** 素材 [[2026-09-18-mental-strength]] 的内容主体由模型产出，无任何来源。本库此前 11 份素材全部是可追溯的外部材料。目前的处理（收录 + 整页降级为 `low` + 显式写明收录理由 + 禁止被其他页面引用）是**一次临场决定，尚未成为正式约定**。见 [[mental-strength]] 的使用限制。
2. **私人内容如何处理？** [[2026-09-18-emotion-awareness-notes]] 含针对北洛具体家庭处境的应对指南。当前处理是「只提炼通用方法、不复述具体描述」，同样**尚未成为约定**。
...
- [ ] **锁定 [[emotion-awareness]] 的 `goal`** —— 三个前提待北洛确认（项目性质 / 验收标准 / 期限），锁定前该页缺口表只是草稿
- [ ] **为「AI 生成内容」立一条本库约定** —— 见上文第二条主线的缺口 1。这是本库第一条此类约定，且很可能不止情绪项目会遇到
- [ ] **核对情绪项目里的五处关键引文** —— 弗兰克尔、索维尔、马可·奥勒留、芒格、弗洛伊德。其中弗兰克尔那句**很可能不是他说的**（见 [[viktor-frankl]]）
